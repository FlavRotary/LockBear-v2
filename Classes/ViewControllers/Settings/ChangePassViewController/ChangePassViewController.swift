...
class ChangePassViewController: BaseViewController, UITextFieldDelegate {
    ...
    //MARK: - Vars
    var commandHandler: ChangePassCommandHandlerProtocol?
    weak var firstResponder: UITextField?
    
    //MARK: - UIVIewController Life Cycle
    
    required init(with commandHandler: ChangePassCommandHandlerProtocol?) {
        super.init(nibName: String(describing: ChangePassViewController.self), bundle: nil)
        self.title = Setting.changePass.getSettingsName()
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        NotificationCenter.default.addObserver(forName: UIResponder.keyboardWillShowNotification, object: nil, queue: nil) { (notification) in
            ...
            if let keyboardSize = (notification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue {
                self.scrollView.contentSize = CGSize(width: self.scrollView.bounds.width, height: self.saveButton.frame.origin.y + self.saveButton.frame.size.height + 20)
    ...
    @IBAction func saveAction(_ sender: Any) {
        ...
        self.commandHandler?.change(passTextField.text!)
    }
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        case confirmPassTextField:
            saveAction(saveButton ?? UIButton())
